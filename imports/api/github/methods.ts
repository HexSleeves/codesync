...
import { type File, Files } from "../files/files";
import { type Session, Sessions } from "../sessions/sessions";
import {
	createOctokit,
	fetchFileContent,
	fetchPRDetails,
...
Meteor.methods({
	...
	async "github.importPR"(prUrl: string, options: { isPublic?: boolean } = {}) {
		...
		try {
			...
			throw new Meteor.Error(
				"github-error",
				error.message || "Failed to import pull request from GitHub",
			);
		}
	},
	...
	async "github.importPRToSession"(sessionId: string, prUrl: string) {
		...
		try {
			...
			throw new Meteor.Error(
				"github-error",
				error.message || "Failed to import pull request",
			);
		}
	},
