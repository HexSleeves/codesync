...
import type { GitHubConnectionStatus, GitHubPRValidationResult, MeteorError } from "../../../types";
...
export const NewSessionForm: React.FC<NewSessionFormProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    Meteor.call("github.checkConnection", (err: MeteorError | undefined, result?: GitHubConnectionStatus) => {
      if (!err && result) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      Meteor.call(
        ...
        githubUrl,
        (err: MeteorError | undefined, result?: GitHubPRValidationResult) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (sourceType === "github" && githubUrl) {
      Meteor.call(
        ...
        githubUrl,
        { isPublic },
        (err: MeteorError | undefined, sessionId?: string) => {
          setLoading(false);
          if (err) {
            setError(err.reason || err.message || 'Unknown error');
          } else if (sessionId) {
                      ...
                      (err) => {
                        if (!err) {
                          ...
                          Meteor.call(
                            ...
                            (e: MeteorError | undefined, result?: GitHubConnectionStatus) => {
                              if (!e && result) setGithubStatus(result);
                            },
                          );
                        }
                      },
