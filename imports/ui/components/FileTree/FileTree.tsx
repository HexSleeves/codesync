  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-wrap gap-1 px-3 py-2 border-b border-gray-200 dark:border-gray-700">
        ...
        <FilterButton active={filter === 'modified'} onClick={() => setFilter('modified')} color="yellow">
          M ({stats.modified})
        </FilterButton>
        <FilterButton active={filter === 'added'} onClick={() => setFilter('added')} color="green">
          A ({stats.added})
        </FilterButton>
        <FilterButton active={filter === 'deleted'} onClick={() => setFilter('deleted')} color="red">
          D ({stats.deleted})
        </FilterButton>
        <FilterButton active={filter === 'unreviewed'} onClick={() => setFilter('unreviewed')} color="blue">
      ...
      <div className="flex items-center gap-2 px-3 py-2 text-xs text-gray-500 dark:text-gray-400">
        ...
        <span className="ml-auto">
          {filteredFiles.length !== files.length ? `${filteredFiles.length}/` : ''}{files.length} files
        </span>
      </div>
        ...
        {filteredTree.length === 0 ? (
          ...
          filteredTree.map(node => (
            <FileNode
              ...
              expandedPaths={searchQuery || filter !== 'all' ? new Set([...expandedPaths, ...getAllPaths(node)]) : expandedPaths}
