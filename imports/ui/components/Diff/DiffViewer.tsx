...
import type { DiffLine, File, Hunk } from "../../../api/files/files";
...
const UnifiedDiff: React.FC<DiffProps> = ({ hunks, comments, onLineClick }) => {
	return (
		<div className="font-mono text-[13px] bg-gray-900 text-gray-100 overflow-x-auto">
...
const DiffLineRow: React.FC<DiffLineRowProps> = ({
	...
	return (
		<div className={`flex group ${bgColor} hover:bg-opacity-70 leading-5`}>
			{/* Line numbers */}
			{showBothNumbers && (
				<>
					<span
						...
						onClick={() =>
							line.oldLineNumber && onLineClick(line.oldLineNumber)
						}
					>
						{line.oldLineNumber || ""}
					</span>
					<span
						...
						onClick={() =>
							line.newLineNumber && onLineClick(line.newLineNumber)
						}
					>
						{line.newLineNumber || ""}
						{hasComments && (
			...
			{/* Prefix */}
			<span className={`inline-block w-4 ${textColor} select-none`}>
				{prefix}
			</span>
